...
- **适配 dsh 0.1.2-rc.1 的预览打开文件（D3）**：0.1.2 把文件打开从 host.openPath 迁到会话控制器的 session/openWorkspacePath（端点 /api/session/openWorkspacePath，payload.args.path）。预览拦截脚本改为同时匹配新旧端点，并从 payload.args.path（回退 payload.path）取路径，恢复「消息流里点文件 → 在预览面板打开」。依据：客户端 createWebConnectionRpc 的端点映射 + 服务端 session.openWorkspacePath 报错信息（待壳内实测确认）。
