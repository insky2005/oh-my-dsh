...
- **升级后未重启服务 / WebView 未重载**：PATH 修复后自动/手动升级能跑完，但运行中的 dsh web 仍在内存里跑旧代码（之前只刷新了版本事实，没重启服务），新版本要到下次启动才生效。修复为升级成功后停止 App 自己拉起的服务并用 `startServer()` 重新拉起 + 重载 WebView（复用对外部已存在服务的复用路径则只重载视图，新版本随下次启动生效）。
