...
- **Channel 面板 ↔ dsh web 会话双向联动**：面板项目视图会话行新增「在 dsh 中打开」，点击经注入的 `sessionOpenerScript` 驱动 dsh web 切换到该会话；反过来 dsh web 切换会话时面板自动展开对应会话、其余收起（无对应全部收起）。**以 sessionId 对应，不用 name**。设计见 `docs/channel-web-session-link.md`。
