...
- **新建工作区**：点「+」只输入目录名（如 `abc`）→ 建 `<root>/abc` → 幂等注册成 dsh 工作区（`workspace/create`；**不需要刷新/重连**，dsh web 侧栏一两秒内自己出现该工作区——工作区流推送，2026-09-24 实测）。注册失败（dsh 未起 / 旧版本 / 注册表读不懂）**只标「未注册」、目录保留**，下次「新会话 / 在 dsh 中打开」会自动再注册；同名目录不是错误（提示「该工作区已存在」并继续注册），名字含 `/`·`:`、以 `.` 开头、空、超 64 字符都会在创建前被拦下；
