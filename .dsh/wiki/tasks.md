...
- **新建工作区**：点「+」只输入目录名（如 `abc`）→ 建 `<root>/abc` → **创建即选中**（卡片立刻高亮，若在视野外自动滚进来；日志 `projects: selected the new workspace …`）→ 幂等注册成 dsh 工作区（`workspace/create`；**不需要刷新/重连**，dsh web 侧栏一两秒内自己出现该工作区——工作区流推送，2026-09-24 实测）。注册失败（dsh 未起 / 旧版本 / 注册表读不懂）**只标「未注册」、目录保留**，下次「新会话 / 在 dsh 中打开」会自动再注册；同名目录不是错误（提示「该工作区已存在」并继续注册），名字含 `/`·`:`、以 `.` 开头、空、超 64 字符都会在创建前被拦下；
...
- **当前工作区只有一个真相**：卡片高亮来自 `ProjectDirectory.current`（在 dsh web 里切会话时面板跟随移动）；**新建/采纳工作区后立刻选中它**（卡片高亮 + 自动滚进视野，各面板重根到该目录），要切别的就点六个入口或「新会话」；
...
- 「新会话」/「在 dsh 中打开」后 web 没切过去：**「新会话」已改为点 dsh web 侧栏该工作区行的 `+`**（桥 `__dshNewSession`，内部重试 12×150ms；dsh 自己复用空会话或新建并打开，**不重连、不堆空会话**），桥失败才兜底 `session/create` + 状态行 `projects.newSessionFallback`；「在 dsh 中打开」仍是点侧栏行 → 桥内部自带重试（标题 8×120ms；会话没标题时改按**工作区分组**点组内第一条，8×150ms）→ 仍需一次外部重试 → **放弃并在状态行说明**（`projects.openFailed` / `openFailedNoSession`）。**永远不会自动重载页面**（2026-09-23 起的 ed989ac：旧版在失败时重载页面，配合 didFinish 重放会形成每 ~10.2s 一轮的死循环刷新）；失败时 `app.log` 会打印原因 + 侧栏现场 `(sidebar: groups=N sessionRows=M workspaceRow=yes/no)`；
