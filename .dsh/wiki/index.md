...
- 最近一次提交：`86abc82`（"feat(wiki): 自动提交 commit message 与内容相关…"）
...
- Wiki 自动提交：生成/更新完成后 `WikiAutoCommit` 自动 `git add` + commit（不 push），commit message 按 porcelain 状态点名新增/更新/删除的页面（86abc82），见 [wiki-panel](modules/wiki-panel.md)
