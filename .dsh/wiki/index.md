...
- **指令体系 v2（未合并分支 `feature/channel-commands-v2`）**：客户端斜杠指令重构——全局 `/help` `/ping` `/status`；工作区指令 `/workspaces`(`/wks`)、`/sessions`(`/ses`) 无内容列出 / 有内容切换（等同 `#wN`/`#sN`）、`/new [内容]` 统一回 `创建新会话 #sN (sessionId)`（无内容建 `New Session` 等首条消息激活、有内容 prompt=内容并回推答案）；删除 `/switch`；core 单测 **171 全绿**。见 docs/channel-commands.md（清单）。
