...
|---|---|---|---|
| 已注册（徽标「已注册 · N 个会话」） | = 在 dsh 中打开 | `plus` 图标，tooltip **「新会话 / New Session」** → 让 dsh web 自己新建/复用会话并切过去（见下节「新会话」） | 可用 |
...
| 流程 | 实测行为 |
|---|---|
| 新建 `abc` | `validateName` → `createDirectory`（**同步**，单个目录创建）→ **`onSelectWorkspace(path)`（main.swift → `adoptProjectDirectory`，创建即选中：卡片立刻高亮）+ `pendingScrollPath`（卡片滚进视野）** → `registerInBackground`（`workspace/create`，幂等）→ 成功后 `onWorkspaceRegistered(path)`（main.swift → `dshWebFollowNewWorkspace`：点侧栏该工作区行的 `+`，让 **dsh web 也切到它**——dsh 自己的「添加工作区」在选完文件夹后就是 `startSession`；没有会话的工作区在 web 里没有可选中项，所以必须起一条会话）→ `reload()`。同名**目录**不是错误：状态行「该工作区已存在」、**同样选中**并**继续注册**；同名**文件** → `projects.createFailed`；被名字规则拒绝时**什么都不做**（不建目录、不改变当前工作区） |
...
## 「新会话」= 点 dsh web 自己的入口（`feature/projects-panel`，2026-09-24）

- **根因（实测 + 读 dsh 客户端源码）**：dsh web 侧栏的会话可见性规则是
  *"Ordinary sessions are visible; among blank sessions, **only the current one is visible**"*
  （`dsh-client-ui-workspace/lib/client.js` 的 `sessionVisible`；`blank` = 从未发过消息，`session/list` 的 `items[].blank`）。
  壳层旧实现用 `session/create` 自己建会话 → 它**不是**页面当前会话 → 侧栏里**根本没有这一行** → 壳层切页面唯一手段是点行 → 「新会话」永远切不过去，还每点一次多留一条打不开的空会话（`app.log` 里 `workspace-row-not-found` + 侧栏 `sessionRows` 不增长，`session/list` 里空会话越积越多）。
- **dsh 自己的语义相反**（`connectWorkspace()`）：先找该工作区已有的 blank 会话 → 命中就**复用** → 没有才建 → 然后 `open()`（它成为当前会话，blank 行随之显示，行文本是本地化的「新会话 / New Session」）。
- **现在的实现**：注入桥新增 `window.__dshNewSession(workspaceName)`（`sessionOpenerScript`）：按工作区名找到侧栏工作区行（`div.projectRow[role="treeitem"][aria-expanded]`，非 `sessionRow`）→ 点行内**最后一枚 button**（行内按钮固定是 [工作区菜单, 新建会话]，最后一枚恒为新建会话；hover 才显示、`x=0 w=0` 也能 `click()`——实测有效）→ 最多重试 12×150ms。日志 `projects: dsh web started a session in <path> (via workspace-new-session)`。
- **收益**：不重连、不堆空会话、完全复用 dsh 的语义；壳层通过 `dshSession` 追踪器跟随页面切过去的会话，`ProjectDirectory` 与面板保持一致。
- **兜底**（侧栏 DOM 变样时）：`register + session/create` 建出会话 + 状态行 `projects.newSessionFallback`（提示去侧栏该工作区行点「+」，那一步会**复用**这条空会话）——不 nudge、不点行。
- **`newestSessionId` 跳过 blank**：blank 会话打不开（见上），返回它等于让「在 dsh 中打开」点了没反应；返回 nil 会让调用方走「新会话」，由 dsh 复用那条空会话。
...
- **有界、无副作用**：尝试（桥内自带重试）→ 最多再重试 1 次（1.5s，`retry` 参数）→ 放弃并把原因写进面板状态行。**不再有任何自动重载页面**，也不再有 offline→online nudge（2026-09-24 起；见失败模式表）。
...
- **创建即选中（壳层 + dsh web 两侧）**（2026-09-24）：① 面板建/采纳工作区后立刻经 `onSelectWorkspace` 把它变成当前工作区（否则用户刚建完却看不到"选中"、还得自己在列表里找），并记下 `pendingScrollPath` → 下一次含该卡片的 `render()` 里 `scrollToVisible` 把它滚进视野；日志 `projects: selected the new workspace <path>`。② 注册成功后 `onWorkspaceRegistered(path)` → `dshWebFollowNewWorkspace` 让 **dsh web 也切过去**（复用「新会话」那条桥：点侧栏工作区行的 `+`，桥内还会 `scrollIntoView` 把该行滚进视野）。原因：**一行工作区行不等于"当前工作区"**——没有会话的 workspace 在 web 里没有可选中项，页面仍停在原来那条会话上，用户看到的就是"卡片亮了、dsh web 没动"。这正是 dsh 自己的行为：`WorkspacePickFlow.onPick` → `startSession(workspaceId)`。
...
| C10b | `session/create`（**仅兜底**） | `{workspaceId}` 优先（会话归属该工作区），被拒退 `{cwd}`（落 Ungrouped，功能可用）；注意 **blank 会话只在它是"当前会话"时可见** | `DshWorkspaceOps.createSession` | 「新会话」没反应 → 退 cwd + 状态行报错；正常路径已改走 B10b |
| C10c | `session/list` | `items[].{sessionId,cwd,running,updatedAt,blank}`（**`blank == true` 的一律跳过**） | `DshWorkspaceOps.newestSessionId` | 字段缺失按「无会话」处理 → 退化为新建（dsh 会复用那条空会话） |
...
| B10/R3 | 侧栏行点击 | `__dshOpenSession(sessionId, workspaceName)` **返回 Promise**（`session/list` 取标题 → 点 `[role="treeitem"].sessionRow`，8×120ms 重试；无标题则按工作区目录名定位分组 → 展开 → 点第一条会话行，8×150ms），resolve `{ok, via: "title"\|"workspace"}` 或 `{ok:false, reason, groups, sessionRows, workspaceFound}` | `sessionOpenerScript` + `openDSHSession`（**必须走 `callAsyncJavaScript`**：`evaluateJavaScript` 读不了 Promise，整个兜底链会一次都不执行） | 切不过去 → 桥内重试 → **最多 1 次外部重试** → 放弃并写状态行 `projects.openFailed` / `openFailedNoSession` + 侧栏现场（**不重载页面、不 nudge**） |
| B10b | 侧栏工作区行的「+」 | `__dshNewSession(workspaceName)` → 工作区行内**最后一枚 button**（hover 才显示，`click()` 有效）→ dsh 的 `connectWorkspace`：复用该工作区 blank 会话，没有才建，然后 open；resolve `{ok, via:"workspace-new-session", button}` | `sessionOpenerScript` + `createSessionInWorkspace` | 行结构/按钮顺序变化或 dsh 改掉复用语义 → 「新会话」点了没反应或越点越多空会话；兜底 RPC + 状态行，`tests/injected-scripts/run.sh` 守脚本可解析与桥名一致 |
...
| 侧栏还没有新工作区 / 新会话的行 | 「新会话」直接点工作区行的「+」（桥内重试 12×150ms，等 dsh 的工作区流把行送到）；点卡片「在 dsh 中打开」失败后 1.5s 再试一次 → 仍失败：**不重载页面、不 nudge**，状态行 `projects.openFailed`（行没找到）/ `projects.openFailedNoSession`（`no-session`）+ 侧栏现场 | `projects: dsh web started a session in …` / `projects: dsh web could not start a session in …` / `openDSHSession <id>: row-not-found … (sidebar: groups=… sessionRows=… workspaceRow=…)` |
