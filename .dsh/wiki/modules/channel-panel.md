...
- **与 dsh web 会话双向联动（2026-08-24，docs/channel-web-session-link.md）**：面板↔web 会话**以 sessionId 对应（不用 name）**。面板 → web：会话行「在 dsh 中打开」（`arrow.up.right.square`，L10n `channel.openInDsh`）→ `onOpenSession(sessionId)` → main.swift 注入的 `sessionOpenerScript` 暴露 `window.__dshOpenSession(id)`（`session.list` 按 sessionId 解析标题 → 展开折叠组 + 异步重试 → 点击侧边栏行）；web → 面板：`dshSession` handler 追加 `channelPanel?.setActiveSession(sid)`，`activeSessionId` 驱动 `rebuildProjectRows`——命中则只展开该会话、其余行收起，未命中则所有会话行收起（channel 列表仍按 enabled 显示，会话可见），无 active 则回落手动折叠状态；
