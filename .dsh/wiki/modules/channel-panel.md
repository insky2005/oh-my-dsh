...
- **dsh 接口双面适配（core/lib/dsh-rpc.js，2026-09-10）**：runner 与 session-driver 不再硬编码 0.1.1 的点号方法。传输层先按 dsh ≥ 0.1.2 的**斜杠端点 + payload.args.<request|_request>** 调用，端点不存在（404）再回退老点号方法，并按**端点**记忆所选接口面（同一个 0.1.2 服务有 session/list 却没有 workspace/list，按服务记忆会互相污染）；`/api` 的 cookie 由 **launch token**（`GET /?token=…`）换取并缓存，token 由壳层经 `channel run --dsh-token`（或环境变量 DSH_WEB_TOKEN）注入，**不落日志**。**工作区列表**在 0.1.2 无 workspace.list → 回退读 dsh 持久化的 `$DSH_HOME/storages/workspace.json`（core/lib/workspace-store.js，按 `global.workspaceIds` 保序，取 workspaceId/path/title/sessionIds），因此 `/wks` 即使拿不到 token 也能列出工作区（**2026-09-10 起该读取加护栏，R4**：`workspace-store.js` 校验 `unit.name/version`，`readWorkspaceStore` 返回 `{items, reason, name, version, file}`，版本不符仍尽力解析但经 `describeStore()` 报出；`channel-runner.js` 的 `loadWorkspaces` 传 `log: m => console.log(m)`，`/wks`、门控、`/ses` 因此不再「静默空列表」，诊断落 `channel-runner-<id>.log`）；**末条回复**改由 `session/page` 按 session/list 的 projection cursor 回放（并过滤 reasoning 片段，不外泄到微信）。壳层侧 ServerManager 记住带 token 的入口地址（`entryURL`/`webToken`），channel runner 改为**服务就绪后**启动；见 docs/plans/dsh-012rc1-compat-audit.md §九；
