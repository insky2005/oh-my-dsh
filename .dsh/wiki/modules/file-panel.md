...
- **L10n 新增键**：`preview.saveHint`（保存当前文件）、`preview.saveFailed`（保存失败：%@）、`menu.file`、`menu.save`；工作区切换询问另加 `preview.switchUnsavedTitle` / `preview.switchUnsavedMessage` / `preview.switchSave` / `preview.switchDiscard` / `preview.switchCancel`（中英成对）；
