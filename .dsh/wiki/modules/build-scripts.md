...
- `install_dsh` / `build_runtime`：用下载的 Node 自带 npm 在 `runtime/dsh` 装 `@deepseek-ai/dsh@0.1.5-rc.2`（默认 `DSH_PACKAGE_SPEC`；壳层与该版本同步适配），主 registry 失败自动重试官方源；`(Node版本|spec|arch)` 写入 `.runtime-info`，相同组合直接复用 `.cache/runtime/<arch>`；
...
| `DSH_PACKAGE_SPEC` | `@deepseek-ai/dsh@0.1.5-rc.2` | npm install 的包说明（内置 dsh 版本，可覆盖为 `@latest` 等） |
