...
5c. **通道（Channel，v1.10 之后 / feature/channel，2026-08-22 合并进 main）**：右栏 `channelPanel`（`ChannelPanelController`）配置全局 channel（微信 ClawBot 等平台卡片 + 扫码登录向导，面板内 `CIQRCodeGenerator` 渲染二维码，调 core `channel login --save` 落 `~/.dsh/channels/<id>.json`）与项目视图（Channel 行 + NSSwitch 开关写项目 `.dsh/channels.json` 引用）；core 层（Node，平台无关）负责全部逻辑——`core/lib/channel.js`（五态状态机 + 路由优先级 + resolveRefBinding + 编排）、`weixin-clawbot*.js`（微信 iLink 官方协议适配器，**严格串行长轮询**防重复回复）、`session-driver.js`（经 dsh RPC 驱动会话，**复用会话**）、`channel-runner.js`（编排 + 指令优先路由 + 异步应答忙门 + 通道级状态 `~/.dsh/channels/<id>.state.json`）、`channel-sessions.js`（**全局**会话映射/消息归档）；数据流：平台入站消息 → adapter.onEvent → ChannelEvent → parseCommand（指令直接回复）或按关联模型路由命中项目（会话复用/新建）→ sendTyping → 后台 session.prompt(mode: queue) → 回推 ChannelReply（带 context_token）+ 取消 typing；详见 [channel-panel](modules/channel-panel.md) 与 [data-model](data-model.md)。
