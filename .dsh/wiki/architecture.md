...
│   └─ 右栏插槽 RightPanel { none, preview, terminal, wiki, tasks, browser, channel }（六面板互斥）   │
...
1. **启动**：`applicationDidFinishLaunching` → **单实例检查**（非开发版且同 bundle id 已有其他实例在跑 → 聚焦它并 `exit(0)`，防双实例争抢 CEF profile 致 Chromium 异常退出；开发版 `isDevBuild` 跳过，见 [main](modules/main.md)）→ `L10n.captureSystemLang()`（在改写 AppleLanguages 前快照系统语言）→ `installSignalHandlers` → `buildMenu` → `SkillInstaller.installBuiltinSkills()`（内置 skill 全局安装，见 [skill-installer](modules/skill-installer.md)）→ `buildWindow` → `startServer` → `startConfiguredChannelRunners()`（拉起已启用全局 channel 的 runner，同 channelId 去重）。`ServerManager.start()`：先查 3080 是否已是 dsh web（`isDSHServing`，页面含 `__DSH_BOOT__`，dsh ≤ 0.1.1）→ 复用；若是 dsh ≥ 0.1.2（`isDSHAuthenticated`：裸 GET 回 401 + `authentication required`）→ **按设计不复用**、记日志 `existing dsh web on 3080 wants its launch token … not adopting`（token 每进程随机且只在该进程 stdout；同 `DSH_HOME` 下数据本就共享）后自拉起（`DSH_NATIVE_FORCE_SPAWN=1` 可跳过复用检查）；否则解析 node/dsh 入口、`node <bin> web --port <n>` 自拉起，轮询直到就绪（90s 超时，含 1s 沉降校验——进程存活且再次可服务才判 up，防引导页假就绪；进程提前退出则抛错）。就绪后 `webView.load`，并通知终端/面板 `serverReady(port:)`。
