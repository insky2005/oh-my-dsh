...
1. **启动**：`applicationDidFinishLaunching` → `L10n.captureSystemLang()`（在改写 AppleLanguages 前快照系统语言）→ `installSignalHandlers` → `buildMenu` → `buildWindow` → `startServer` → `startConfiguredChannelRunners()`（拉起已启用全局 channel 的 runner，同 channelId 去重）。`ServerManager.start()`：先查 3080 是否已是 dsh web（`isDSHServing`，页面含 `__DSH_BOOT__`）→ 复用；否则解析 node/dsh 入口、`node <bin> web --port <n>` 自拉起，轮询直到就绪（90s 超时，含 1s 沉降校验——进程存活且再次可服务才判 up，防引导页假就绪；进程提前退出则抛错）。就绪后 `webView.load`，并通知终端/面板 `serverReady(port:)`。
