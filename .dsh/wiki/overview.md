...
platforms/macos/src/main.swift       壳层核心（日志/L10n/服务管理/升级/窗口/菜单/设置窗口/onboarding/CoreBridge）4961 行（实测 4961）
...
platforms/macos/src/PreviewPanel.swift  预览面板回滚基线 + 共享 UI 组件（1519 行；`DynamicFillView` 脏矩形夹取修复见 [preview-panel](modules/preview-panel.md)）；现行预览实现为 FilePanel.swift
...
tests/               无头单元测试（terminal-emulator/、wiki-panel/、browser-panel/、channel-panel/、dsh-rpc/、review-panel/、file-panel/、terminal-panel/、l10n/、shell-config/、dsh-auth-cookies/、skills/、skills-panel/，各含 run.sh；模拟器测试已迁 core/tests/ansi.test.js 的薄封装）
