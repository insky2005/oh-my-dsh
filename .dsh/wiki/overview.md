...
- 内置运行时构建期现做：下载 Node tarball（默认国内镜像 `npmmirror.com/mirrors/node`，校验 SHA-256），用其自带 npm 在 `runtime/dsh` 装 `@deepseek-ai/dsh@0.1.2-rc.1`（默认 `DSH_PACKAGE_SPEC`，国内源失败自动回退 npmjs.org）；
