...
oh-my-dsh 是 DeepSeek Harness 的 **macOS 原生壳**：把 `dsh web`（`@deepseek-ai/dsh` 的浏览器界面）封装成可双击运行的 `.app`。**不改动任何 DeepSeek Harness 源码**，只做端口探测、服务拉起/复用、WKWebView 呈现，并附右栏面板（预览/文件、终端、Repo Wiki、任务、浏览器、通道、审查、技能）。
...
当前工作区版本：`1.17.0`（fallback，BUILD 73；最新发布 tag **`v1.16.1`**（2026-09-23 发布，release/1.16 patch：内置 dsh 升到 0.1.5-rc.2 + 审查面板会话日志世代命名修复），另有 `v1.16.0`（2026-09-21）与 `v1.13.1`（release 分支 patch））；版本由 git tag 驱动（`scripts/version.sh`：HEAD 命中 vX.Y.Z → 取 tag，否则回退 1.17.0；BUILD 取 CI 运行号）。内置 dsh 版本：`@deepseek-ai/dsh@0.1.5-rc.2`（`DSH_PACKAGE_SPEC` 默认值；v1.16.1 起从 0.1.2-rc.1 升级，审计见 `docs/plans/dsh-015rc2-compat-audit.md`）。当前分支：**`main`**（HEAD `b3451dd` = v1.16.0 发布 + fallback 推进；最近一次功能合并仍是 **PR #52 `feature/ux-feedback-fixes`**——**使用问题清单（`docs/ux-feedback.md`）九条集中落地**：Files 面板目录树右键菜单（新建文件夹/文件 → 重命名 → 删除（废纸篓）→ 在 Finder 中显示，判定抽成纯模型 `FilePanelTreeMenu.swift`）+ 头部改为 **`PanelMenuButton`「打开项目 ▾ / 打开文件 ▾」**（`OpenWithApps.swift` 提供 Finder/13 编辑器 IDE/8 终端目录，记忆键 `files.openProjectWith`）+ **图片预览自适应与手动缩放**（`ImagePreviewView.swift` + 纯模型 `ImageZoom.swift`：5%–1600%、⌘+/−/0、⌘滚轮、捏合、双击、拖拽平移、居中留边距 + 固定尺寸角标）+ **大文件不再关高亮而是分块着色**（`EditorLoadPolicy.swift`：300 行/32 KB 分块 + 0.6s 写文件稳定性窗口 + 后台读取，安全阀只对 >4 万行 / >4 MB）+ **目录树宽度跨关闭恢复**（`TreeDividerSplitView`，前两版因事件监视器拿不到分隔条 tracking loop 而失效）+ Terminal 面板**滚动方向对齐面板语义**、**双击选词/三击选行 + 选中即复制**（`terminal.autoCopy`，默认开）、**页签按 workspace 隔离**（`TerminalWorkspaceTabs.swift`，隐藏不杀会话）+ Shell **⌘R 重新加载页面 = 走 launch token 重新认证 + 主框架 401 自愈**；此前合并 **PR #51 `feature/panel-surface-colors`**——**面板配色统一**：新增 `platforms/macos/src/PanelSurface.swift`（`PanelSurface` 面板底色 `#1B1B1C`/`#F9FAFB` + `PanelControl` 控件两档 `#43454A`/`#353638`，浅色 `#FFFFFF`/`#F1F3F5`），八个面板的 header / 内容区 / 列表底色与卡片、按钮、页签配色全部收敛到这一处，`DynamicFillView.Kind` 由 `.window`/`.control` 改为 `.panel`/`.custom`，方案文档 `docs/ui-color-scheme.md`；此前合并 **PR #50 `feature/skills-manager`**——**技能面板（Skills / ⌥⌘S）落地**：新增 `SkillsPanel.swift` / `SkillsCore.swift` / `SkillSources.swift`，管理 dsh 四个技能根的**查找 / 安装 / 移除**与 `user-invocable` / `disable-model-invocation` 调用开关（写回 SKILL.md frontmatter、切回默认即删键还原字节），内置技能只读、共享级可改开关不可移除；registry 可配置（skills.sh 搜索型 / well-known 清单 / GitHub 仓库清单），并修掉 `DynamicFillView` 越界填充致头部被遮（2119bb1）+ 滚动后 hover 残留（6dd6c0e）；配套 `tests/skills-panel/` 121 项与 `docs/skills-manager-design.md`。此前合并 **PR #49 `fix/review-audit-staleness`**——审查面板审计结果按**日志身份（size + mtime）**失效、在屏时每 5 s stat 增量重审，新建会话不再「只显示会话、看不到文件」；`df7c163` 发布 **v1.15.0**（[Unreleased] 转正 + SECURITY/README/CONTRIBUTING 同步），`d9affe1` 发布后 fallback 推进到 **1.16.0**（BUILD 72）。此前合并 **PR #48 `fix/browser-blank-panel`**——**浏览器面板空白修复收尾**：① `ShellConfig` 补旧 UserDefaults 一次性迁移并把渲染默认改回文档记载的 `windowed`（407ccb1，配套 `tests/shell-config/` 13 例）；② OSR 自绘帧改画在可见的 `pageView` 层 + 菜单锚点/帧派发按 `browserId`（9a39fe0）；③ **dsh 认证 cookie 无限累积**修掉——新增 `platforms/macos/src/DshWebCookieJanitor.swift`（启动清非本次 authority 的 `dsh-auth-*`、退出清本次的，`NODE_OPTIONS` 追加 `--max-http-header-size=65536`），修「启动后 Failed to load plugins」（8374736，配套 `tests/dsh-auth-cookies/` 22 例）。此前 **PR #44 `feature/review-panel`**——**审查面板（只读变更审计）落地**：core `review-log` 读 dsh 会话日志（Zstandard 多帧）折叠出「会话→对话→文件→变更内容」树，右栏第七面板（⌥⌘R）+ `ReviewLogModel` 展示模型 + `tests/review-panel`；设计见 docs/review-panel-design.md。此前 `5348c0d` 发布 **v1.14.0**（[Unreleased] 转正 + SECURITY/README/CONTRIBUTING 同步），`2996b06` 发布后 fallback 推进到 1.15.0；`ebac11a` 用 `--test-timeout=60000` 修掉泄漏用例挂死整套）。此前 HEAD `aad6daa`，合并 **PR #39 `feature/channel-dingtalk-stream`**——**钉钉原生适配器落地**（M4 提前）：`dingtalk-stream`（Stream 长连接）适配器 + device-code 扫码自动创建应用（AppKey/AppSecret）+ `/bind` owner-binding 安全门（未绑定拒绝所有人）+ 面板钉钉扫码向导/绑定页 Re-login/unbind；与微信共享同一 channel store 与项目视图，core 单测 **186 全绿**，见 [channel-panel](modules/channel-panel.md) 与 docs/channel-dingtalk-stream.md）。此前合并 PR #36 `fix/upgrade-and-restart`——**自动升级修复**：npm 升级子进程前置注入内置 node 目录到 `PATH`（修复 GUI 启动 PATH 无 node 致 lifecycle 脚本 exit 127），升级成功后停止自拉起的服务并重拉 + 重载 WebView 使新 dsh 立即生效）。此前合并 PR #30 `feature/channel-project-switch`——**Channel「项目开关」落地**：通道↔项目（workspace）启用关联存全局 `~/.dsh/channels/<channelId>.workspaces.json`，开关真正门控路由（普通消息/`/new`/`#wN`/`#sN`/`/sessions`/`/switch` 对未启用 workspace 回「该项目未启用该通道」、不建会话，`/workspaces` 只列已启用；`/help` `/ping` `/status` 放行），见 [channel-panel](modules/channel-panel.md) 与 docs/channel-project-switch.md。此前 PR #28 `feature/channel-association` 落地 **Channel 关联模型**（会话复用 A / 路由统一 B / 工作区归属 C / 存储全局化 D / 面板项目视图 E）、异步应答 + 忙门 + sendTyping、channel runner 日志落盘。此前 PR #27 `feature/builtin-skills-global` 合并 **内置 Skill 全局化**（SkillInstaller 装到全局 $DSH_HOME/skills/、shell-browser→web-dev-tools / repo-wiki→repo-knowledge / issue-fix→issue-resolve 重命名）；PR #23 `feature/channel` 把**通道能力合入 main**：微信 ClawBot 扫码登录 → 长轮询收消息 → 斜杠指令 / #tag 路由 → 驱动 dsh 会话 → 回复回微信，全链路真实微信端到端验证；macOS 面板 v2（引导卡片 + 扫码向导 + 项目视图 + 状态徽标）。此前的文件面板强化已合入（`feature/file-panel` HEAD `6b5aab4`：预览面板改由 [FilePanel](modules/file-panel.md)（`FilePanelController`）实现，新增无后缀/点文件按文本预览、文件内编辑 + 行号栏、Highlightr 语法高亮，及「文件」菜单（保存 ⌘S / 关闭页签 ⌘W））。此前最近合并进 main 的提交：`8f51161`（合并 PR #14 feature/browser-panel）——浏览器面板内核定为 CEF/Chromium（root cause：CEF 148+ 需五 helper app，缺 `(Renderer)` 致 renderer 静默失败，见 [browser-panel](modules/browser-panel.md)）；浏览器面板演进已合并进 main（OSR 离屏渲染默认、Chromium 原生 DevTools 窗口、控制台抽屉移除、QA 端点 debug/hierarchy、DevTools 拖动修复）。发布/CI 侧近期演进：`local-release.sh`（含 `pack` 只打包不发布子命令）、`github-publish.sh`/`release-checksums.sh` 发布脚本、`Jenkinsfile`、runtime/CEF 缓存按架构分目录（`.cache/runtime/<arch>`、`.cache/cef-built-<arch>`）、release.yml 新增 prepare 前置 job，见 [build-scripts](modules/build-scripts.md) 与 [tasks](tasks.md)。历史：Node 选择策略定型为**系统优先、内置兜底、带版本门槛**（ac4312c 系统优先：resolveNode = DSH_NODE > 系统 node（PATH→nvm current→nvm default→nvm 最新→Homebrew，90d9176/ceefc0c 定序）> 内置 node；6428249 加版本门槛：候选低于 22.0.0 跳过（`DSH_NODE_MIN` 可覆盖），全过旧则内置兜底，版本比较内联 Swift 不经 CoreBridge 避免递归；启动轮询加 1s 沉降校验防引导页假就绪；f4cfa33：dsh web 环境经 `loginShellPath()`（`/bin/zsh -ilc` 读一次登录 shell PATH，8s 超时兜底、结果缓存）合并用户 PATH，不注入内置目录）；About 面板显示实际 node 路径（8fcd6cc）并将 Node 版本+路径合并为一行（e9d6bb2）；此前任务面板工作区识别改**权威判断**（workspacePath 非 GitHub 仓库时诚实显示空态、不再替换为其他工作区，13bf9e3；切换到不同仓库先清空旧... (line truncated to 2000 chars)

## 目录布局

```
core/                共享核心（Node 模块：ANSI 模拟器 / 端口探测 / 升级 / 会话 RPC / issues / jobqueue / tasks 关联索引 / dsh RPC 传输层 / shell 设置 / workspace 存储 / channel 通道（抽象+路由+指令+存储+微信 ClawBot 与钉钉 dingtalk-stream 适配器+会话驱动）/ review-log 会话日志变更审计（zstd 多帧解码），跨平台复用；随构建嵌入 runtime/core）
...
docs/                设计与排查文档（repo-wiki-design.md、review-panel-design.md（审查面板设计/覆盖矩阵）、skills-manager-design.md（技能面板：四档级别判定 / registry 模型 / 开关可逆写法）、productization.md、git-workflow.md、release-process.md、ui-color-scheme.md（面板配色方案：令牌表 / 取色 API / 应用映射）、dsh-version-impact.md、milestones/、plans/、terminal-header-fix.md、terminal-input-fix.md、channel-*.md、raw/）
tests/               无头单元测试（terminal-emulator/、wiki-panel/、browser-panel/、channel-panel/、dsh-rpc/、review-panel/、file-panel/（6 个测试文件，含真实 NSWindow 场景）、terminal-panel/、l10n/、shell-config/、dsh-auth-cookies/、skills/、skills-panel/，各含 run.sh；模拟器测试已迁 core/tests/ansi.test.js 的薄封装）
...
- 内置运行时构建期现做：下载 Node tarball（默认国内镜像 `npmmirror.com/mirrors/node`，校验 SHA-256），用其自带 npm 在 `runtime/dsh` 装 `@deepseek-ai/dsh@0.1.5-rc.2`（默认 `DSH_PACKAGE_SPEC`，国内源失败自动回退 npmjs.org）；
...
```bash
node --test --test-timeout=60000 core/tests/*.test.js   # 共享核心单测（ANSI 模拟器 / 端口 / 升级 / 会话 RPC / dsh RPC / settings / workspace-store / issues / jobqueue / tasks / channel / review-log，233 用例；不带引号由 bash 展开 glob，Node 20 兼容；--test-timeout 让泄漏定时器的用例 60s 失败而非挂死整套）
...
tests/dsh-rpc/run.sh                  # 壳层原生 dsh RPC（信封形状 / 斜杠↔点号回退 / launch token 换 cookie）
...
- core 单测为 Node 测试（`core/tests/*.test.js`：ansi 42 / ports 5 / session 4 / upgrade 10 / issues 8 / jobqueue 7 / tasks 7 = 83 + 其余（channel 相关 / dsh-rpc / workspace-store / settings / review-log 等）= **233 用例，2026-09-12 本机实测 230 通过 / 3 跳过**（Node v20.19.6 无 zstd，review-log 的 zstd 用例自动 skip）；channel 覆盖 channel/commands/runner/sessions/workspaces/weixin-clawbot/dingtalk/e2e-channel/channel-association/channel-busy/project-switch）；`tests/terminal-emulator/run.sh` 现为 `core/tests/ansi.test.js` 的薄封装；
