...
- core 单测为 Node 测试（`core/tests/*.test.js`：ansi 42 / ports 5 / session 4 / upgrade 4 / issues 8 / jobqueue 7 / tasks 7 = 77 + channel 相关 89 = **166 全绿**，2026-08-24 实测；channel 覆盖 channel/commands/runner/sessions/workspaces/weixin-clawbot/e2e-channel/channel-association/channel-busy/project-switch）；`tests/terminal-emulator/run.sh` 现为 `core/tests/ansi.test.js` 的薄封装；
