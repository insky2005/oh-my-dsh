...
oh-my-dsh 是 DeepSeek Harness 的 **macOS 原生壳**：把 `dsh web`（`@deepseek-ai/dsh` 的浏览器界面）封装成可双击运行的 `.app`。**不改动任何 DeepSeek Harness 源码**，只做端口探测、服务拉起/复用、WKWebView 呈现，并附右栏面板（预览/文件、终端、Repo Wiki、任务、浏览器、通道、审查）。
...
platforms/macos/src/main.swift       壳层核心（日志/L10n/服务管理/升级/窗口/菜单/设置窗口/onboarding/CoreBridge）约 4770 行（实测 4766）
...
platforms/macos/src/ReviewPanel.swift    审查面板（只读变更审计：会话→对话→文件→变更内容 树 + 按需审计缓存 + 跟随 dsh web）1031 行
platforms/macos/src/ReviewLogModel.swift 审查面板展示模型（纯 Foundation：JSON 解码 / 文件分组 / turn 分组 / diff 折叠）353 行
...
docs/                设计与排查文档（repo-wiki-design.md、review-panel-design.md（审查面板设计/覆盖矩阵）、productization.md、git-workflow.md、release-process.md、dsh-version-impact.md、milestones/、plans/、terminal-header-fix.md、terminal-input-fix.md、channel-*.md、raw/）
tests/               无头单元测试（terminal-emulator/、wiki-panel/、browser-panel/、channel-panel/、dsh-rpc/、review-panel/、file-panel/、terminal-panel/、l10n/、shell-config/、dsh-auth-cookies/、skills/，各含 run.sh；模拟器测试已迁 core/tests/ansi.test.js 的薄封装）
...
tests/review-panel/run.sh             # 审查面板展示模型（JSON 解码 / 文件与 turn 分组 / diff 折叠；64 项）
