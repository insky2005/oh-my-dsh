...
docs/                设计与排查文档（repo-wiki-design.md、productization.md、git-workflow.md、milestones/、plans/、terminal-header-fix.md、terminal-input-fix.md、channel-design.md、channel-commands.md、channel-status.md、channel-storage.md、channel-project-switch.md、channel-ui-commands.md、channel-issues.md、raw/）
...
```bash
node --test core/tests/*.test.js     # 共享核心单测（ANSI 模拟器 / 端口 / 升级 / 会话 RPC / issues / jobqueue / tasks / channel 通道，187 用例；不带引号由 bash 展开 glob，Node 20 兼容）
...
- core 单测为 Node 测试（`core/tests/*.test.js`：ansi 42 / ports 5 / session 4 / upgrade 4 / issues 8 / jobqueue 7 / tasks 7 = 77 + channel 相关 110 = **187 全绿**，2026-08-28 实测；channel 覆盖 channel/commands/runner/sessions/workspaces/weixin-clawbot/dingtalk/e2e-channel/channel-association/channel-busy/project-switch）；`tests/terminal-emulator/run.sh` 现为 `core/tests/ansi.test.js` 的薄封装；
