...
- 版本号单一来源：`scripts/version.sh`（HEAD 命中 git tag vX.Y.Z → VERSION 取 tag，否则回退 1.17.0；BUILD 取 CI 运行号，本地回退 73；最新发布 tag **v1.16.0**）；`platforms/macos/build-app.sh` 运行期读取，**勿在脚本硬编码版本**；产物命名 `oh-my-dsh-<version>-<arch>.{pkg,dmg}`（`platforms/macos/make-pkg.sh` 从 Info.plist 读取版本，避免两处失配）；
...
- **版本支持策略**：维护最近 2 个大版本；主版本（vX.Y.0）发布时同步更新 `SECURITY.md` ## Supported Versions（新版本进表、最老出表、EOL 边界上移；patch 版本表不变跳过），见 [tasks](tasks.md)。**当前表**（v1.16.0 发布后）：`1.16.x` ✅ / `1.15.x` ✅ / `< 1.15` ❌ End of life。

## 面板 UI 约定

- 右栏八个面板共享 `PreviewPanel.swift` 的 UI 基件（`HoverButton`/`DynamicFillView`/`CustomIconButton`/`HeaderLabel` 等），视觉与交互保持一致：40pt 头部 + 内容区、统一背景条、图标深浅色均可见；浏览器面板在其上扩展 `CustomIconButton`（`size`/`hoverColor`，见 [preview-panel](modules/preview-panel.md)）做 Chrome 式页签与关闭按钮；
