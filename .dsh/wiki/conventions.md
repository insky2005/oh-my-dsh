...
- 生成/更新完成后由**代理**执行 `git add .dsh/wiki` + commit（**不 push**），commit message 由代理概括实际变更（提交约定来自更新请求与面板 `WikiPrompts.fallbackInstructions`，repo-wiki SKILL.md 不再内置提交规则）；若代理未提交，面板 `WikiAutoCommit` **兜底**提交（同样不 push），提交失败仅记日志不打扰用户；
