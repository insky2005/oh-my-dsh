...
final class IssueRunnerPanelController: NSObject, NSTableViewDataSource, NSTableViewDelegate {
    ...
    func startTask(number: Int) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            // 0. ensure the issue-fix skill exists in this workspace (idempotent)
            _ = Self.ensureIssueFixSkillInstalled(repoRoot: path)
    ...
    // MARK: - issue-fix skill installation & prompt

    /// Install the issue-fix skill into <repoRoot>/.dsh/skills/issue-fix/ if
    /// missing (idempotent, mirrors WikiSkill.ensureInstalled). Ensures any
    /// workspace — even a fresh project that isn't this repo — can process
    /// GitHub issues with the same skill.
    @discardableResult
    static func ensureIssueFixSkillInstalled(repoRoot: String) -> Bool {
        let dir = (repoRoot as NSString).appendingPathComponent(".dsh/skills/issue-fix")
        let path = (dir as NSString).appendingPathComponent("SKILL.md")
        do {
            try FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true)
            if let existing = try? String(contentsOfFile: path, encoding: .utf8),
               existing == issueFixSkillMarkdown {
                return true
            }
            try issueFixSkillMarkdown.write(toFile: path, atomically: true, encoding: .utf8)
            AppLog.shared.log("issue-fix skill installed: {path}")
            return true
        } catch {
            AppLog.shared.log("issue-fix skill install failed: {error.localizedDescription}")
            return false
        }
    }

    /// Embedded copy of .dsh/skills/issue-fix/SKILL.md (kept byte-identical so
    /// ensureIssueFixSkillInstalled never overwrites a newer repo copy).
    static let issueFixSkillMarkdown = """
    ---
    name: issue-fix
