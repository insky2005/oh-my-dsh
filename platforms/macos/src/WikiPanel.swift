...
//      preserved)
//    - Generation: repo-wiki skill install + session.create/prompt RPC
...
// MARK: - repo-wiki skill (installed into the repo so the agent discovers it)

enum WikiSkill {

    /// Installed at <repoRoot>/.dsh/skills/repo-wiki/SKILL.md (rank 100 in the
    /// dsh skill filesystem provider's discovery order). Written only when
    /// missing or changed; never touches dsh source or dsh home.
    static func ensureInstalled(repoRoot: String) -> Bool {
        let dir = (repoRoot as NSString).appendingPathComponent(".dsh/skills/repo-wiki")
        let path = (dir as NSString).appendingPathComponent("SKILL.md")
        do {
            try FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true)
            if let existing = try? String(contentsOfFile: path, encoding: .utf8), existing == skillMarkdown {
                return true
            }
            try skillMarkdown.write(toFile: path, atomically: true, encoding: .utf8)
            AppLog.shared.log("wiki skill installed: \(path)")
            return true
        } catch {
            AppLog.shared.log("wiki skill install failed: \(error.localizedDescription)")
            return false
        }
    }
...
enum WikiPrompts {
    ...
    static func prompt(kind: Kind, repoRoot: String, wikiRoot: String, zh: Bool) -> String {
        ...
        if zh {
            return """
            请加载 repo-wiki skill 并执行【\(target)】。
            仓库根目录：\(repoRoot)
            知识库输出目录：\(wikiRoot)

            若 repo-wiki skill 不可用，请按以下内嵌说明执行：
        ...
        return """
        Load the repo-wiki skill and run \(target).
        Repository root: \(repoRoot)
        Wiki output directory: \(wikiRoot)

        If the repo-wiki skill is unavailable, follow these inline instructions:
...
final class WikiPanelController: NSObject, NSOutlineViewDataSource, NSOutlineViewDelegate,
    ...
    private func startGeneration(_ kind: WikiPrompts.Kind) {
        ...
        // Make sure the skill the agent should load is installed.
        _ = WikiSkill.ensureInstalled(repoRoot: repo)
