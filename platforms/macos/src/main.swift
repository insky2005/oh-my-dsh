...
enum L10n {
    ...
    private static let table: [String: (zh: String, en: String)] = [
        ...
        // Files 面板：目录树重命名 / 删除（#1 后续）。
        "files.rename": ("重命名", "Rename"),
        "files.renameAction": ("重命名", "Rename"),
        "files.renameFailed": ("重命名失败：%@", "Rename failed: %@"),
        "files.delete": ("删除", "Delete"),
        "files.deleteAction": ("移到废纸篓", "Move to Trash"),
        ...
        "preview.closeUnsavedMessage": ("关闭会丢失以下文件的未保存修改：\n%@", "Closing will discard unsaved changes in:\n%@"),
        "preview.switchSave": ("保存并切换", "Save and Switch"),
        "preview.closeSave": ("保存并关闭", "Save and Close"),
        "preview.discard": ("不保存", "Don't Save"),
        // terminal panel
        ...
        "skills.registry.title": ("registry 管理", "Registries"),
        "skills.registry.back": ("返回", "Back"),
        "skills.registry.enabled": ("启用", "Enabled"),
        "skills.registry.empty": ("暂无 registry", "No registries yet"),
        "skills.registry.added": ("registry 已添加", "Registry added"),
        ...
        "scaffold.projectSummary": ("项目简介（一句话）", "Project summary (one line)"),
        "scaffold.projectSummaryPlaceholder": ("例如：内部 API 网关", "e.g. internal API gateway"),
        "scaffold.willCreateDir": ("将创建目录", "Will create folder"),
        "scaffold.willCreateDirHint": ("请先填写项目名并选择位置", "fill in a name and pick a location"),
        ...
        // 环节管理设置（右上角 ⚙）
        "scaffold.settingsTitle": ("环节管理", "Stage settings"),
        "scaffold.stageBuiltin": ("内置", "Built-in"),
        "scaffold.stageModified": ("自定义 · 已修改", "Custom · modified"),
        "scaffold.stageNew": ("自定义 · 新建", "Custom · new"),
        "scaffold.newStage": ("新建环节", "New stage"),
        "scaffold.editStage": ("编辑", "Edit"),
        "scaffold.restoreStage": ("恢复", "Restore"),
        "scaffold.deleteStage": ("删除", "Delete"),
        "scaffold.moveUp": ("上移", "Move up"),
        "scaffold.moveDown": ("下移", "Move down"),
        "scaffold.editorTitleEdit": ("编辑环节：%@", "Edit stage: %@"),
        "scaffold.editorTitleNew": ("新建环节", "New stage"),
        "scaffold.back": ("返回", "Back"),
        "scaffold.save": ("保存", "Save"),
        "scaffold.saveAll": ("保存全部", "Save All"),
        "scaffold.editorHint": ("stage.yaml 定义环节；templates/ 下为渲染模板（{{var}}/{{#if}} 语法）。修改内置环节保存后自动复制到用户库（可恢复）。", "stage.yaml defines the stage; files under templates/ are render templates ({{var}}/{{#if}} syntax). Editing a built-in stage copies it to the user library on save (restorable)."),
        "scaffold.openInFinder": ("在 Finder 中打开环节库", "Show stage library in Finder"),
        "scaffold.newTemplate": ("新建模板", "New template"),
        "scaffold.templateNamePrompt": ("模板文件名（将创建到 templates/ 下）：", "Template file name (created under templates/):"),
        "scaffold.templateNameInvalid": ("文件名无效：不能为空、不能含 / 或 ..", "Invalid file name: must be non-empty, no / or .."),
        "scaffold.templateNameTaken": ("templates/ 下已存在同名文件", "A file with that name already exists under templates/"),
        "scaffold.saveYamlFirst": ("请先保存 stage.yaml（确定环节 id）后再保存模板文件", "Save stage.yaml first (to set the stage id) before saving template files"),
        "scaffold.settingsFooter": ("自定义环节保存在 %@", "Custom stages are stored in %@"),
        "scaffold.stageYAMLError": ("保存失败：%@", "Save failed: %@"),
        "scaffold.stageIDInvalid": ("环节 id 只能包含小写字母、数字和连字符", "Stage id may only contain lowercase letters, digits and hyphens"),
        "scaffold.stageIDTaken": ("环节 id 已存在：%@", "Stage id already exists: %@"),
        "scaffold.stageIDChanged": ("环节 id 不可修改（如需改名请新建环节）", "Stage id cannot be changed (create a new stage to rename)"),
        "scaffold.stageSaveOK": ("已保存：%@", "Saved: %@"),
        "scaffold.stageRestored": ("已恢复内置：%@", "Restored built-in: %@"),
        "scaffold.stageDeleted": ("已删除：%@", "Deleted: %@"),
        "scaffold.stageReorderOK": ("排序已更新", "Order updated"),
        "scaffold.confirmRestoreTitle": ("恢复内置环节？", "Restore built-in stage?"),
        "scaffold.confirmRestoreMessage": ("将删除自定义版本「%@」，恢复到内置环节定义。", "This deletes the custom version of “%@” and restores the built-in stage definition."),
        "scaffold.confirmDeleteTitle": ("删除环节？", "Delete stage?"),
        "scaffold.confirmDeleteMessage": ("将删除自定义环节「%@」，此操作不可撤销。", "This deletes custom stage “%@”. This cannot be undone."),
        "scaffold.presetStageCount": ("%d 个环节", "%d stages"),
        "scaffold.settingsPresetTab": ("项目预设", "Project presets"),
        "scaffold.settingsStageTab": ("环节管理", "Stage settings"),
        "scaffold.newPreset": ("新建预设", "New preset"),
        "scaffold.editPreset": ("编辑", "Edit"),
        "scaffold.restorePreset": ("恢复", "Restore"),
        "scaffold.deletePreset": ("删除", "Delete"),
        "scaffold.presetBuiltin": ("内置", "Built-in"),
        "scaffold.presetModified": ("自定义 · 已修改", "Custom · modified"),
        "scaffold.presetNew": ("自定义 · 新建", "Custom · new"),
        "scaffold.presetEditorNew": ("新建项目预设", "New project preset"),
        "scaffold.presetEditorEdit": ("编辑项目预设：%@", "Edit project preset: %@"),
        "scaffold.presetFieldID": ("id", "id"),
        "scaffold.presetFieldNameZh": ("名称（中文）", "Name (Chinese)"),
        "scaffold.presetFieldNameEn": ("名称（English）", "Name (English)"),
        "scaffold.presetFieldDescZh": ("描述（中文）", "Description (Chinese)"),
        "scaffold.presetFieldDescEn": ("描述（English）", "Description (English)"),
        "scaffold.presetChooseStages": ("环节（按序勾选）", "Stages (select in order)"),
        "scaffold.presetStagePickerEmpty": ("从上方选择环节加入预设", "Pick stages above to add"),
        "scaffold.presetParamHint": ("为选中环节设置参数默认值（可为空）", "Set default params for chosen stages (may be empty)"),
        "scaffold.presetAdd": ("添加环节 +", "Add stage +"),
        "scaffold.presetRemove": ("移除", "Remove"),
        "scaffold.presetSaveOK": ("已保存预设：%@", "Saved preset: %@"),
        "scaffold.presetRestored": ("已恢复内置预设：%@", "Restored built-in preset: %@"),
        "scaffold.presetDeleted": ("已删除预设：%@", "Deleted preset: %@"),
        "scaffold.presetReorderOK": ("预设排序已更新", "Preset order updated"),
        "scaffold.presetIDInvalid": ("预设 id 只能包含小写字母、数字和连字符", "Preset id may only contain lowercase letters, digits and hyphens"),
        "scaffold.presetIDTaken": ("预设 id 已存在：%@", "Preset id already exists: %@"),
        "scaffold.presetEmptyName": ("请填写预设名称", "Please fill in the preset name"),
        "scaffold.confirmRestorePresetTitle": ("恢复内置预设？", "Restore built-in preset?"),
        "scaffold.confirmRestorePresetMessage": ("将删除自定义版本「%@」，恢复到产品内置预设。", "This deletes the custom version of “%@” and restores the product built-in preset."),
        "scaffold.confirmDeletePresetTitle": ("删除项目预设？", "Delete project preset?"),
        "scaffold.confirmDeletePresetMessage": ("将删除自定义预设「%@」，此操作不可撤销。", "This deletes custom preset “%@”. This cannot be undone."),
        "scaffold.settingsFooterPreset": ("自定义预设保存在 %@", "Custom presets are stored in %@"),
        "scaffold.presetMissingStage": ("（缺失环节 %@）", "(missing stage %@)"),
        "scaffold.presetNoName": ("预设名称为空", "Preset name is empty"),
        "scaffold.presetEditRestoreHint": ("恢复内置", "Restore built-in"),
        "scaffold.presetMoveUp": ("上移", "Move up"),
        "scaffold.presetMoveDown": ("下移", "Move down"),
        "scaffold.presetApply": ("套用此预设", "Apply this preset"),
        "scaffold.presetAddFirstStage": ("先勾选上方环节再加入", "Check stages above first"),
