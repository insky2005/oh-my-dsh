...
enum L10n {
    ...
    private static let table: [String: (zh: String, en: String)] = [
        ...
        "skills.installSelected": ("安装选中（%d）", "Install selected (%d)"),
        "skills.selectAll": ("全选", "Select all"),
        "skills.clearSelection": ("清空", "Clear"),
        "skills.bulkDone": ("批量安装完成：成功 {ok}，失败 {fail}", "Bulk install done: {ok} installed, {fail} failed"),
