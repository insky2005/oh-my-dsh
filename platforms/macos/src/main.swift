...
enum L10n {
    ...
    private static let table: [String: (zh: String, en: String)] = [
        ...
        "tasks.configInfo": ("GitHub token（存 Keychain；或写入 ~/.dsh/gh-token 供 App 与外部工具共用）。仅用于拉取 issues、创建 PR、评论关闭 issue；公开仓库可留空。", "GitHub token (stored in Keychain; or write ~/.dsh/gh-token so the app and external tools share it). Used only to fetch issues, create PRs, comment & close issues; public repos may leave empty."),
