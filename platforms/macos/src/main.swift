...
enum L10n {
    ...
    private static let table: [String: (zh: String, en: String)] = [
        ...
        "scaffold.editorID": ("环节 ID：%@（YAML 中不可修改）", "Stage ID: %@ (cannot change in YAML)"),
        "scaffold.editorIDHint": ("环节 ID：请编辑 YAML 中的 id（小写字母/数字/连字符）", "Stage ID: edit the id in YAML (lowercase letters/digits/hyphen)"),
        "scaffold.editorHint": ("模板文件（templates/）：修改内置环节时自动复制到用户库；新建/自定义环节请点下方按钮在 Finder 中放入。", "Template files (templates/): copied to the user library automatically when editing a built-in stage; for new/custom stages, use the button below to open the folder in Finder."),
