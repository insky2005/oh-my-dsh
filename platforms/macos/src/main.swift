...
enum L10n {
    ...
    private static let table: [String: (zh: String, en: String)] = [
        ...
        "scaffold.editorHint": ("提示：修改内置环节时，模板文件（templates/）会随定义一起复制到用户环节库。新建环节如需模板文件，请手动放入环节目录。", "Tip: when editing a built-in stage, template files (templates/) are copied into the user library with the definition. For new stages, place template files into the stage folder manually."),
