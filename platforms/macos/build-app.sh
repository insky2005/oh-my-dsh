...
#      @deepseek-ai/dsh@0.1.5-rc.2; override with DSH_PACKAGE_SPEC).
#
...
install_dsh() {
  local target="$1"
  local spec="${DSH_PACKAGE_SPEC:-@deepseek-ai/dsh@0.1.5-rc.2}"
  mkdir -p "$target"
  ( cd "$target" \
...
if [ "$MODE" = "prefetch" ]; then
  ...
  echo "Node: $NODE_VERSION | dsh: ${DSH_PACKAGE_SPEC:-@deepseek-ai/dsh@0.1.5-rc.2}"
