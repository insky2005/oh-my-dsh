  ...
  const commandRunner = opts.commands || createCommandRunner({
    ...
    createSession: async (content) => {
      ...
      if (content) {
        ...
        // /new with NO content: create only (no dsh run), title "New Session", mark
        // pending — the next ordinary message activates it. Unified ack + 请继续.
        await sessionDriver.renameSession(port, sid, 'New Session', wsHost, opts.timeoutMs);
