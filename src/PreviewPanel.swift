//
//  PreviewPanel.swift — Right-side file/folder preview panel for oh-my-dsh.
//
...
/// A borderless icon button with a hover highlight, a pointing-hand cursor,
/// and a distinct highlight while in the "on" (selected) state. Used by the
/// activity bar and the preview panel's header buttons.
class HoverButton: NSButton {

    /// Whether hover/press feedback should be shown (disabled buttons stay flat).
...
/// An activity-bar toggle button with the symbol color BAKED into the image
/// (SymbolConfiguration(paletteColors:)) — no contentTintColor dependency, so
/// icons stay visible in dark mode even in layer-backed hierarchies. The
/// active state tints with the accent color; inactive uses a mode-aware gray.
final class ActivityBarButton: HoverButton {
    ...
    private func refreshIcon() {
        ...
        if isActive {
            ...
            color = dark ? NSColor(white: 0.8, alpha: 1) : NSColor(white: 0.3, alpha: 1)
        }
...
/// An image view whose symbol is rendered with an explicit mode-aware color
/// BAKED into the image (SymbolConfiguration(paletteColors:)), refreshed on
...
final class BakedIconView: NSImageView {
    ...
    private func refresh() {
        let dark = effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
        let color: NSColor = dark ? NSColor(white: 0.8, alpha: 1) : NSColor(white: 0.35, alpha: 1)
        image = NSImage(systemSymbolName: symbolName, accessibilityDescription: nil)?
            .withSymbolConfiguration(NSImage.SymbolConfiguration(paletteColors: [color]))
    }
}

/// An icon button whose symbol tint is resolved EXPLICITLY against the
/// button's effective appearance (light gray on dark, dark gray on light),
/// refreshed whenever the appearance changes. Unlike a dynamic
/// contentTintColor, this never depends on NSAppearance.current resolving
/// correctly inside a layer-backed hierarchy — which can otherwise leave
/// icons dark-on-dark (invisible) in dark mode.
final class PanelIconButton: HoverButton {
    private let symbolName: String

    init(symbol: String, tooltip: String, target: AnyObject?, action: Selector) {
        self.symbolName = symbol
        // Use the DESIGNATED initializer + property assignment. Calling
        // NSButton's convenience init(title:target:action:) through a Swift
        // subclass dispatches the class factory +buttonWithTitle:target:action:
        // on the instance, which crashes with an unrecognized selector.
        super.init(frame: .zero)
        title = ""
        self.target = target
        self.action = action
    ...
    private func refreshTint() {
        // Defer appearance resolution until the button is actually in a
        // window (viewDidMoveToWindow re-resolves once mounted).
        guard window != nil else { return }
        let dark = effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
        // Resolve the icon color and BAKE it into the symbol image
        // (SymbolConfiguration(paletteColors:)). This never depends on
        // contentTintColor / template-tint resolution, which can leave icons
        // dark-on-dark (invisible) in layer-backed hierarchies.
        let color: NSColor = dark ? NSColor(white: 0.9, alpha: 1) : NSColor(white: 0.22, alpha: 1)
...
/// A header label drawn entirely with Core Graphics text rendering (the same
/// path the terminal's text uses, which renders reliably everywhere).
...
final class HeaderLabel: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let dark = effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
        let color: NSColor = dark ? NSColor(white: 0.78, alpha: 1) : NSColor(white: 0.38, alpha: 1)
...
final class CustomIconButton: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        case .folder:
            path.move(to: NSPoint(x: inset.minX, y: inset.minY + 3))
            path.line(to: NSPoint(x: inset.minX + 9, y: inset.minY + 3))
            path.line(to: NSPoint(x: inset.minX + 13, y: inset.minY + 7))
            path.line(to: NSPoint(x: inset.maxX, y: inset.minY + 7))
            path.line(to: NSPoint(x: inset.maxX, y: inset.maxY))
            path.line(to: NSPoint(x: inset.minX, y: inset.maxY))
            path.close()
        case .openInApp:
            path.move(to: NSPoint(x: inset.minX + 1, y: inset.maxY - 1))
            path.line(to: NSPoint(x: inset.maxX - 1, y: inset.minY + 1))
            path.move(to: NSPoint(x: inset.maxX - 5, y: inset.minY + 1))
            path.line(to: NSPoint(x: inset.maxX - 1, y: inset.minY + 1))
            path.line(to: NSPoint(x: inset.maxX - 1, y: inset.minY + 5))
        case .reveal:
...
final class PreviewPanelController: NSObject, NSTableViewDataSource, NSTableViewDelegate,
    ...
    private func buildUI() {
        ...
        let actions = NSStackView(views: [projectButton, openButton, revealButton, hideButton])
    ...
    private func showFile(_ path: String) {
        ...
        if isMarkdown || type.conforms(to: .text) || Self.textExtensions.contains(ext) {
    ...
    private func showText(path: String, data: Data) {
        ...
        AppLog.shared.log("preview text: \(textView.string.count) chars")
        embed(scroll)
    }
