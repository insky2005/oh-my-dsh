...
**dsh 运行时（`@deepseek-ai/dsh@0.1.0-rc.6`）侧已具备的能力：**
...
## 附：相关资源
- dsh 包（`@deepseek-ai/dsh@0.1.0-rc.6` 依赖树内）：`dsh-agent-instructions`（AGENTS.md 加载）、`dsh-skill` / `dsh-skill-filesystem`（skill 注册与 5 级根）、`dsh-host-apiproxy`（`sessions.schema.js`：session.create/prompt/list 请求/响应 schema）、`dsh-session-query-sqlite`（FTS5 会话检索）、`dsh-workspace`、`dsh-settings-file`
