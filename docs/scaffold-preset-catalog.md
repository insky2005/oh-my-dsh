...
- 环节：git-init + git-conventions + agents-md + docs-standards + coding-conventions + repo-knowledge。
...
- 环节：git-init + git-conventions(enforce=true) + agents-md + coding-conventions + docs-standards + makefile(按栈填命令) + ci-cd(hasBackend 按需) + repo-knowledge。
...
- 环节（现状 foundation 可落地部分）：git-init(node) + git-conventions + agents-md(ts/node) + coding-conventions + docs-standards + makefile(lang=node) + ci-cd(hasFrontend=true hasBackend=false) + docker(runtime=static, exposePort=80)。
...
- 环节：git-init(license=Apache-2.0) + git-conventions(enforce) + coding-conventions + docs-standards + makefile(test/lint) + ci-cd + repo-knowledge。
...
- 环节：git-init + git-conventions + agents-md + coding-conventions + makefile(lang 按脚本语言)；不含 ci/docker/deploy（如需门禁再加 ci-cd）。
- must-have：Makefile 统一 install/build/test/lint/run 入口；不生成 Dockerfile/部署（避免过度生成）。
- golden：待生成。

### I. etl-data 数据 / 批处理【草稿】
- 场景：企业数据管道 / 批处理仓（Python）。
- 环节：git-init + git-conventions + agents-md + coding-conventions + docs-standards + makefile(lang=python) + ci-cd(校验/单测门禁)；不含 docker/deploy（非 web 服务）。
...
- 环节：git-conventions(enforce) + coding-conventions + agents-md + docs-standards + ci-cd(镜像占位+发布门控) + repo-knowledge + makefile(统一目标约定)；是否含 docker/deploy 视团队部署方式。
