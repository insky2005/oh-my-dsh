...
| B10 | **Projects 面板点开会话**：沿用 B5/B6 的 `__dshOpenSession` 侧栏行点击（会话 id → `session/list` 取标题 → 点 `[role="treeitem"].sessionRow`）；壳层刚建好的会话在客户端重拉列表前还没有对应行 | 同 R3（DOM / 传输改动即静默失效） | 面板「新会话」后 web 不切过去（console 里 `[dsh-opener] row-not-found`） | `platforms/macos/src/main.swift`（`openDSHSession(_:retry:)` + `pendingOpenSessionId` + `nudgeDSHWebCaches`） | `DSH_UI_DEBUG=1` 的 `dsh injected bridges` + console `[dsh-opener]` |
...
| C10b | **Projects 面板建会话**：`session/create { workspaceId\|cwd }`（先 workspaceId 保证分组，被拒退 cwd） | 同 C7 | 面板「新会话」无反应 | `DshWorkspaceOps.createSession` | 同上 + `tests/projects-panel/run.sh` |
...
- [ ] C 项目面板：在「项目」面板新建一个工作区 → 刷新/重连后 dsh web 侧边栏出现该工作区（覆盖 **C10a**）；点「新会话」→ web 切到该工作区的新会话（覆盖 **C10b**）；再点卡片名称「在 dsh 中打开」→ 复用刚建的那条会话（覆盖 **C10c / B10**）。
