...
| D4 | **活动栏放第一位**；快捷键 **⌥⌘P** | 顺序：项目、文件、终端、知识库、任务、通道、审查、浏览器、技能（**2026-09-24 起「浏览器」由第 4 位挪到末位「技能」之前**，其余不变）；「视图」菜单首项同为「项目」，与活动栏一一对应。⌥⌘P 原先属于「文件面板」，本次**让位**给它（文件面板改 **⌥⌘F**，菜单文案「预览面板」改「文件面板」）——见 §13.E |
...
2. **第二行**：绝对路径（次要色、中间截断、tooltip 全路径）+ **紧跟其后的两个路径工具**：在 Finder 中显示（`.reveal`）、复制路径（`link`）——它们是"这个目录在哪"的附属动作，故与路径同行（2026-09-24 从操作行上移，并按用户要求**紧贴路径文本、不右对齐**：行内尾部放一个低 hugging 的弹性占位吸收余量，路径标签 hugging 高、压缩阻力低，因此窄卡片先截断路径而不是挤掉按钮）。
...
- 六个入口、「新会话」与**新建/采纳工作区**都通过同一个重根原语写它，因此**面板高亮、终端页签归属、任务/通道/审查的工作区**不可能互相打架；
