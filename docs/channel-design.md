...
- **传输层回归（2026-08-21）**：core/tests/e2e-channel.test.js 用 **mock HTTP** 覆盖 ClawBot iLink 传输层（getupdates 映射 / -14 过期 token / 无 token / sendmessage 报文 / QR 登录），**不依赖真实 dsh web、不做会话创建**；session run CLI 为纯会话链路调试命令（不涉微信）。
