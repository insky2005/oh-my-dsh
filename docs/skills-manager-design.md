...
可用列表每张卡前的勾选框 = **多选**，配合底部「安装选中（N）」（N 为选中数，无选中时禁用）与「全选 / 清空」：

- 批量安装把选中的候选**逐个**装进同一个目标根（弹窗里选用户级 / 项目级），逐条在状态栏报进度（i/N）；
- 同名已存在 → **按失败计数、不覆盖**（批量不做逐个确认，避免弹出 N 个对话框）；结束后状态栏给「成功 {ok} / 失败 {fail}」并列出失败项，日志里也有明细；
- 成功后清空选择并触发 web 端技能菜单刷新（见 §6.1）。
