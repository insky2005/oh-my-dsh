...
### P4 落地形态
- 内置预设：在仓库 scaffold-presets/<id>/preset.yaml 增文件（id + 双语 name/desc + stages 有序列表 + params 默认值；文件夹布局同内置 stage；模板只属于 stage，preset 只组合 stage，随 App 进 Contents/Resources/scaffold-presets），加文档卡与 golden。
