...
- **dsh 依赖锁定**：`DSH_PACKAGE_SPEC` 固定 RC 版本（当前 `@deepseek-ai/dsh@0.1.2-rc.1`），升级走显式发布流程（→ §8.3），不做 daily 追新
...
| dsh 上游 | `DSH_PACKAGE_SPEC` 固定（当前 `0.1.2-rc.1`） | npm 原地升级（现有机制，独立于 App） |
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| 上游 dsh 仍为 RC（`0.1.2-rc.1`） | API/行为可能变化 | 版本锁定 + 兼容矩阵 + nightly 冒烟；紧跟上游 Release |
